...
export function renderModal({ id, image, title, content, buttonText, buttonAction }) {
    return `
        <div id="${id}" class="modal-overlay hidden">
            <div class="modal-content glass-card p-8 max-w-sm mx-auto mt-[10vh]">
...
export function renderLockOverlay(message = '登录后查看下载链接', actionText = '去登录', action = 'navigate-profile') {
    return `
...
export function renderMenuItem({ icon, label, value, action, params, isLast, hasArrow = true }) {
    ...
    return `
        <div ${actionAttr} ${paramsAttr} class="p-4 ${borderClass} flex justify-between items-center cursor-pointer active:bg-gray-50 dark:active:bg-gray-800/50 transition-colors">
            ...
            ${value ? `<span class="text-xs ${value === '已绑定' ? 'text-green-500' : ''}">${escapeHtml(value)}</span>` : ''}
    ...
    return `
        <div class="flex gap-3 items-start">
            ...
            <div class="flex-1">
                ...
                <button class="mt-2 bg-pink-600 text-white text-xs px-4 py-2 rounded-full btn-active">发送</button>
            </div>
        </div>
    `;
}
    ...
    return `
        <div class="skeleton-card" style="animation-delay: 0s">
            ...
            <div class="skeleton-content">
                <div class="skeleton-text skel-base" style="width:75%;margin-bottom:8px"></div>
