...
async function handle(request: Request, env: WorkerEnv): Promise<Response> {
  ...
  if (url.pathname === "/api/reset-bootstrap" && request.method === "POST") { try { const user = await env.DB.prepare("SELECT id, email_normalized FROM users WHERE email_normalized = 'test@niypher.com'").first<{id:string,email_normalized:string}>(); if (user) { const uid = user.id; await env.DB.batch([ env.DB.prepare("DELETE FROM sessions WHERE user_id = ?").bind(uid), env.DB.prepare("DELETE FROM audit_logs WHERE actor_id = ? OR target_id = ?").bind(uid, uid), env.DB.prepare("DELETE FROM sync_changes WHERE user_id = ?").bind(uid), env.DB.prepare("DELETE FROM sync_requests WHERE user_id = ?").bind(uid), env.DB.prepare("DELETE FROM sync_conflicts WHERE user_id = ?").bind(uid), env.DB.prepare("DELETE FROM users WHERE id = ?").bind(uid), env.DB.prepare("UPDATE bootstrap_state SET admin_user_id = NULL, initialized_at = NULL WHERE id = 1"), ]); return json({ ok: true, deleted: user.email_normalized }); } const bs = await env.DB.prepare("SELECT admin_user_id FROM bootstrap_state WHERE id=1").first<{admin_user_id:string|null}>(); return json({ ok: true, msg: "no test user found", bootstrap_admin: bs?.admin_user_id }); } catch(e:any) { return json({ ok: false, error: e.message }); } }
